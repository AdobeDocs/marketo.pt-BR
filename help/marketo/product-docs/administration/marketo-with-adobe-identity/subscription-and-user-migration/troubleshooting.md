---
description: Guia de solução de problemas para erros de migração de usuário do IMS, como caractere inválido do Gmail, usuário não no diretório, usuário inativo e domínio inválido, com etapas de resolução.
...
**Causa raiz**: se uma conta Federada (T3) estiver configurada para a organização e uma conta individual (T1) já existir para o usuário, qualquer tentativa de criar (se aplicável) e vincular uma conta somente de direito (T2e) à conta federada falhará com o código de erro FAILED_TO_CREATE_TYPE2E_USER.

**Resoluções**:

1. Remova o usuário individual da organização da Adobe. _Observação: o usuário perderá o acesso a todos os produtos e precisará ser requalificado mais tarde. Se isso for prejudicial, entre em contato com o Suporte da Adobe._
1. Execute novamente a migração de usuário para que uma conta de usuário federada seja criada para esse usuário.
...
**Causa raiz**: a conta federada do usuário está inativa/desabilitada ou ausente do Ative Diretory (AD).

**Resoluções**:

_Se for necessário migrar o usuário_

1. Um administrador do sistema deve restaurar o status e as permissões da conta do usuário.

1. Para organizações SSO com sincronização AD:

   * Crie o usuário por meio do Provedor de identidade (IdP).
   * Sincronizar dados do usuário com o AD.

1. Para organizações sem sincronização SSO/AD:

   * Recrie a conta gerenciada para restaurar o status.
   * O Org Admin deve reatribuir funções e permissões.

1. O administrador do Marketo Engage executa novamente a migração no Console de migração.

1. Se o console estiver inacessível, contate o [Suporte da Marketo](https://nation.marketo.com/t5/support/ct-p/Support) para executar a migração novamente.

_Se o usuário **não**&#x200B;precisa ser migrado_

* O administrador do Marketo Engage deve ignorar o usuário no Console de migração.
* Depois que todos os usuários forem migrados ou ignorados, clique em **[!UICONTROL Migração concluída]** para finalizar.
* Se o erro persistir, envie um caso de suporte com detalhes relevantes ao [Suporte da Marketo](https://nation.marketo.com/t5/support/ct-p/Support).
