---
unique-page-id: 7513141
description: Saiba como navegar na página inicial do Analytics para acessar diferentes tipos de relatório e retornar à exibição inicial. Selecione tipos de relatório e gerencie seu espaço de trabalho de relatórios.
title: Navegação na página inicial do Analytics
exl-id: 51e044ac-c761-4d51-ac8a-168ba8f763c6
feature: Reporting
TQID: https://experienceleague.adobe.com/5Ty5tJPKBWkb5L-thDAwarjmDn3TbNaLyjjRKk5Wvf0
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: ea90ebee-5c84-42d9-8b21-006bdabc95a3
topic_v2:
  - id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 78
ht-degree: 14%

---

# Navegação na página inicial do Analytics {#navigating-the-analytics-home-page}

1. Vá para a área **[!UICONTROL Analytics]**.

   ![](assets/image2015-4-27-8-3a38-3a10.png)

1. Selecione um [tipo de relatório](/help/marketo/product-docs/reporting/basic-reporting/report-types/report-type-overview.md).

   ![](assets/image2015-4-27-8-3a38-3a22.png)

1. Depois de executar o relatório, clique no espaço de trabalho para voltar para a **Página inicial do Analytics**.

   ![](assets/image2015-4-27-8-3a38-3a34.png)

   Ótimo! Você sabe como navegar pela página inicial do Analytics!

>[!MORELIKETHIS]
>
>[Entendendo meus relatórios e relatórios de grupo](/help/marketo/product-docs/reporting/basic-reporting/creating-reports/understanding-my-reports-and-group-reports.md)
