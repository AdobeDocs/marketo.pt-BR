---
unique-page-id: 2359498
description: Saiba como visualizar um trecho para verificar o conteúdo de cada segmento. Use o Design Studio e a Visualização de trecho para verificar o conteúdo dinâmico antes da aprovação.
title: Pré-visualizar um snippet
exl-id: 757e14ef-af7b-44d8-9c73-89b281df3d42
feature: Snippets
TQID: https://experienceleague.adobe.com/r4xN3mAn27zDB4Uwp7lZ1mzsU5iAd8bQNYrJREfVQpk
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: d65b4a73-87a3-4d56-b638-74e74d9939ce
  - id: ed6be6bb-75bb-4ea9-9a42-3bcaa65e1bcc
subfeature_v2:
  - id: a1d50dda-6d94-4e16-8c30-5eb7181c4650
  - id: cdd4e0f6-e87e-453f-88ee-2ee54a7de272
  - id: df8eb12b-4f82-491f-acbb-d74012ca5654
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 76
ht-degree: 7%

---

# Pré-visualizar um snippet {#preview-a-snippet}

>[!PREREQUISITES]
>
>[Adicionar conteúdo a um trecho](/help/marketo/product-docs/personalization/segmentation-and-snippets/snippets/add-content-to-a-snippet.md)

Os trechos são blocos de conteúdo dinâmico que mudam com base nas regras de segmentação.

1. Vá para o **[!UICONTROL Design Studio]**.

   ![](assets/designstudio-3.png)

1. Clique no seu trecho e depois em **[!UICONTROL Visualizar trecho]**.

   ![](assets/image2014-9-16-9-3a48-3a32.png)

A visualização é excelente para garantir que o conteúdo esteja correto para cada segmento.

>[!MORELIKETHIS]
>
>[Aprovar um trecho](/help/marketo/product-docs/personalization/segmentation-and-snippets/snippets/approve-a-snippet.md)
