---
unique-page-id: 7504060
description: Excluir uma apresentação - Documentos do Marketo - Documentação do produto
title: Excluir uma apresentação
exl-id: 027d6d68-c2b6-4424-bc69-77d047d62359
translation-type: tm+mt
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '50'
ht-degree: 0%

---

# Excluir uma apresentação {#delete-a-presentation}

Às vezes você pode ter uma coisa boa demais. Exclua apresentações antigas para abrir espaço para novas apresentações!

1. Selecione uma apresentação.

   ![](assets/image2015-3-26-12-3a26-3a41.png)

1. Clique com o botão direito do mouse e selecione **Delete**.

   ![](assets/image2015-3-26-12-3a26-3a51.png)

1. Clique em **Delete** para confirmar.

   ![](assets/image2015-3-20-16-3a21-3a10.png)

   Pobre! Fora de vista, fora de mente.
