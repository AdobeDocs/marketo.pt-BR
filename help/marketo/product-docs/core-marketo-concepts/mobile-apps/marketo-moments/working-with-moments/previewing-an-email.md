---
unique-page-id: 8783563
description: Pré-visualização de email - Documentos do Marketo - Documentação do produto
title: Visualização de um email
exl-id: 10bcedbd-553f-4ba1-b1bd-1aad2890dbd4
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '67'
ht-degree: 0%

---

# Visualização de um email {#previewing-an-email}

Clique com o botão direito do mouse em um cartão de email para visualizá-lo, antes de puxar o acionador.

1. Em um cartão de email, toque no menu de ação de três pontos.

   ![](assets/image2015-9-25-11-3a30-3a52.png)

1. Toque **Visualizar Email**.

   ![](assets/image2015-7-14-16-3a42-3a21.png)

1. Você pode exibir o email em seu dispositivo.

   ![](assets/image2015-9-25-11-3a23-3a42.png)

   >[!NOTE]
   >
   >Para enviar uma amostra diretamente da página Visualização de email, toque no ícone de avião de papel no canto superior direito.

   Excelente!
