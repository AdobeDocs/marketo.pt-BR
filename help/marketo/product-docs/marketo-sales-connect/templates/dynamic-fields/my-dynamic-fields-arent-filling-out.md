...
* Verifique se ele está conectado corretamente ao Sales Connect. Vá para a [Configurações](https://toutapp.com/login) e clique em **Gerenciar** ao lado do seu CRM.

**Solução de problemas de campos básicos e personalizados**

Tout Campos básicos: ex. `{{company}}`

Tout Campos personalizados: ex.: `{{custom_field_favorite_movie}}`
...
>[!NOTE]
>
>A variável `{{first_name}}` procura tanto no Sales Connect quanto no Salesforce para tentar obter informações. Todos os outros campos desta lista só estão procurando no Sales Connect para preencher o campo.
