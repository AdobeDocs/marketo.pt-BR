...
description: Criar um evento com Webex - Documentos do Marketo - Documentação do produto
...
Primeiro crie um Evento Webex no Webex Event Center. O Marketo usa apenas configurações e campos específicos para sua integração, que passaremos em breve. Outros campos que você pode configurar para Webex são explicados na variável [Guia do usuário do Webex Event Center](https://www.cisco.com/c/dam/en/us/td/docs/collaboration/meeting_center/wbs298/wx_ec_host_ug.pdf).
...
As opções a seguir são usadas ou visualizadas no Marketo. Outros campos residem somente no Webex.

* **Descrição** - Insira uma descrição. Isso será visível, mas não modificável no Marketo.
* **Pesquisa pós-evento** - A Marketo não pode capturar as informações em uma pesquisa pós-evento do Webex no momento.
...
1. Em **Logon**, escolha o logon do Webex.
...
>[!NOTE]
>
>Para alterar seu agendamento de evento, você precisará editar o webinário no Webex.
