...
Na Personalização da Web, um visitante pode corresponder a mais de um segmento. Por exemplo, se houver um segmento para visitantes dos EUA e um segmento para empresas financeiras, um visitante da Web do Bank of America corresponderia **tanto** ao segmento para visitantes dos EUA quanto ao segmento para empresas financeiras.
...
* **Selecione Estado ou província a ser adicionado** - Na caixa suspensa, selecione o estado dos EUA ou província canadense que deseja incluir. É possível fazer várias seleções.
...
>[!TIP]
>
>**Quais condições de segmento eu escolho? &quot;AND&quot; ou &quot;OR&quot;? **OU funciona como uma opção adicional em cada campo. Os prospectos precisam apenas atender a um critério dos vários critérios selecionados em cada campo para se qualificarem para o segmento. (Por exemplo, prospectos podem ser dos EUA. *ou* da indústria da defesa). E funciona como um parâmetro obrigatório adicional que deve ser atendido para este segmento. (Por exemplo, os prospectos devem ser dos EUA e da indústria de Defesa). Em cada perfil de segmentação, cada campo separado pode funcionar como ambos, seja &quot;E&quot; ou &quot;OU&quot;, dependendo da Condição do segmento selecionada.

**Setores** Na seção Segmentação **do** Perfil, marque a caixa ao lado de **Indústria**.

* Selecione um dos seguintes parâmetros:

   * **Inclui** - Selecione se deseja que o segmento inclua ou exclua um setor.
   * **Selecionar setores para adicionar** - Selecione o setor que deseja incluir no segmento. O setor é exibido abaixo da caixa suspensa. Você pode escolher vários setores.
...
* 
   ...
   * **É dessas empresas (nomes específicos)**
      ...
      * Você pode digitar o nome exato da organização que deseja público alvo. *É sempre *recomendado usar Listas de conta nomeada em vez de digitar os nomes manualmente para obter melhores correspondências (consulte abaixo).*

**Lista de conta nomeada**

Selecione a partir de uma Lista [Conta](../../../product-docs/web-personalization/account-based-web-marketing/create-a-new-account-list.md) nomeada para segmentar as principais contas direcionadas

![](assets/image2015-5-27-17-3a14-3a8.png)

>[!NOTE]
>
>O número entre colchetes ao lado do nome da Lista Conta nomeada é usado como uma referência de índice para a lista da API [de](http://developers.marketo.com/documentation/websites/rtp-js-api/)leitura de personalização da Web.
...
Selecione Banco de dados e selecione um campo de dados de pessoa no menu suspenso. Selecione **+** para adicionar campos na lista suspensa.
...
**Segmento de Campanha** de email do Marketo e personalize campanhas por email de um visitante que clica em um email do Marketo e chega ao site. Segmentar por Nome do Programa ou Nome da Campanha do Marketing e continuar a conversa de email para Web. Selecione + para adicionar campos no menu suspenso.
...
**Referências** - Adicione URLs pelos quais o visitante foi referenciado.
...
**Incluir páginas** - Rastrear prospectos de páginas específicas visitados em seu site.
...
**Excluir páginas** - exclui páginas específicas que não deseja corresponder no segmento. (É permitido usar * como curinga).
...
* **Tipo de Visitante **
   **Mobile OS** - Na caixa suspensa, selecione um ou mais SOs móveis listados. O SO móvel selecionado é exibido abaixo.
...
A chamada da API de personalização da Web [lê mais sobre ela aqui.](http://developers.marketo.com/documentation/websites/rtp-user-context-api/)

>[!TIP]
>
>**Usando curingas - **Quando você deseja incluir qualquer termo de pesquisa ou URL que contenha algo dentro dele, por exemplo: &quot; [google.com](http://google.com)&quot; ou &quot;produto de termo de pesquisa&quot;, chamamos isso de curinga e ele deve ser digitado com um asterisco - esse carinha* - em cada extremidade. Portanto, qualquer coisa vinda do [google.com](http://google.com) deve ser inserida como * [google.com](http://google.com)*
